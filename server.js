...
// This line is only ever required if we let the user upload files.
...
{/* 9 times out of 10 this two lines below is all we need */}
// If the browser is sending json to the server, we can easily access that data
app.use(express.json());
// if someone is sending a instead of an asyncronous request if they just submit a traditional old fastioned HTML form we wanna be able to easily access that data as well
...
app.post('/create-animal', upload.single('photo'), ourCleanup, async (req, res) => {
    if (req.file) {
        ...
        // 844px wide and 456px tall
        await sharp(req.file.buffer).resize(844, 456).jpeg({quality: 60}).toFile(path.join(path.join('public', 'uploaded-photos', photofilename)));
...
app.post('/update-animal', upload.single('photo'), ourCleanup, async (req, res) => {
    if (req.file) {
        ...
        if(info.value.photo) {
            fse.remove(path.join('public', 'uploaded-phots', info.value.photo));
        }
        res.send(photofilename)
    } else {
        // if they are not uploading photo
...
async function start() {
    const client = new MongoClient("mongodb://localhost:27017/mern-project-01?authSource=admin");
